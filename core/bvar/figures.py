"""
core/bvar/figures.py

LE FIGURE DEL GATE 6 — un involucro, non un disegnatore.

    python -m core.bvar.figures

Questo modulo NON disegna: chiama `core/forecast/figures.py`, che e' gia' la
...
Sono due tagli della stessa passata, e la differenza e' l'ASSE X.
  ...
  rmse per orizzonte   x = `horizon_week`, con le TRE FASI come bande di
         ...
         Vive in `core/bvar/metrics.py` perche' e' una figura di metrica (la
...
from core.forecast import figures as dfm
...
def discover_csvs(paths: list[str] | None = None) -> list[str]:
    ...
    if not found:
        raise SystemExit(
            f"Nessun CSV in {d}.\n"
            f"Generane uno con:  python -m core.bvar.evaluate "
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Figure del Gate 6 — involucro su core/forecast/figures.py")
