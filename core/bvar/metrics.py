"""
core/bvar/metrics.py

LE METRICHE DEL GATE 6 — RMSE, NY Fed, log score.  Involucro, non calcolatore.

    python -m core.bvar.metrics

Tre uscite, due delle quali sono `core/forecast/` chiamato con altri argomenti.
I percorsi li decide `core/output_layout.py`, non questo modulo:
...
Nessun file di `core/forecast/` e' modificato.  Cio' che e' nuovo qui e' solo il
...
from core.bvar.evaluate import OUTPUT_ROOT
from core.bvar.figures import MODELS, discover_csvs
from core.forecast import compare_nyfed as cn
from core.forecast import compute_metrics as cm
from core.forecast.nyfed_nowcast import load_long as load_nyfed
...
def logscore_tables(df: pd.DataFrame, out_dir: str) -> pd.DataFrame:
    ...
    """
    from core.forecast.metrics_tables import (common_points,
