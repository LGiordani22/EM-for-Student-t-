...
sostengono si rifanno con `python -m core.bvar.tests.test_mixing`.
...
from core.bvar.dummies import initial_observation_mean, sum_of_coefficients_dummy
from core.bvar.niw import build_prior, log_ml_with_dummies
...
def build_target(spec: BVARSpec, panel: np.ndarray,
    ...
    if np.isnan(panel).any():
        raise ValueError(
            ...
            "(vedi core/bvar/data.py::assert_dense)."
        )
    p, n = spec.p, spec.n
    T = panel.shape[0]
    Y = panel[p:]
