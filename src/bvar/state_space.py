...
PRIMITIVE IMPORTATE da `src/kalman.py`.  Il filtro non viene riscritto: si
importano `kalman_predict`, `kalman_update` e `build_selection_matrix`, che sono
pura algebra matriciale, e si scrive qui il solo ciclo.  `src/kalman.py` NON si
...
Cosa si importa da `src/kalman.py`, e nient'altro:
...
from src.bvar.cube_root import MonthlyMap
from src.kalman import build_selection_matrix, kalman_predict, kalman_update
...
# =================================================================
# `src/kalman.py` NON si tocca: e' condiviso con `em/`, `mcmc/`, `forecast/`, e
...
def kalman_update_fast(f_pred: np.ndarray, P_pred: np.ndarray, y_t: np.ndarray,
    ...
    """
    `src.kalman.kalman_update` riscritto nell'ordine di `runKF_DK.m`.
