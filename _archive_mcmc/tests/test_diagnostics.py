"""
src/mcmc/tests/test_diagnostics.py
==================================

COPERTURA (parametri/percorsi -- notazione del README/.tex)
-----------------------------------------------------------
NON copre parametri del modello: copre le FUNZIONI diagnostiche e la loro
correttezza --
  * corr(Q), coupling-overconfidence, attenuazione del whitening di rho
    (forme chiuse verificate CONTRO Monte Carlo, non contro se stesse);
  * ESS, split-R-hat;
  * la garanzia che le diagnostiche non consumino RNG (draw bit-identici on/off).
E' lo strumento con cui si LEGGONO gli altri gate (es. "nessun rho_hat senza il suo
ESS"), non un gate di recovery.

Gate per le **metriche diagnostiche** -- the pure functions of ``Q``
(and of the draws) made *measurable* at every run instead of asserted once:
  ...
  [1] ``posterior_corr_Q``  — the single number that closes all three: the decoupled
      ...
      benefit and the Branch-B leverage attenuation are second-order in the
      off-diagonals of ``corr(Q)``.  It is also the Huang--Wand robustness check the
      thesis prescribes (``.tex`` ~20684).

  [2] ``coupling_overconfidence``.  Ignoring the log-square cross-correlation
  ...
  [3] ``leverage_whitening_attenuation``.  The closed form
...
def test_posterior_corr_Q():
    print("\n[1] posterior_corr_Q -- letto dai draw (scale-invariante)")
...
def test_coupling_overconfidence():
    print("\n[2] coupling_overconfidence -- secondo ordine in corr(Q)")
...
def test_leverage_whitening_attenuation():
    print("\n[3] leverage_whitening_attenuation -- forma chiusa vs Monte Carlo")
    ...
    # le proprieta' su cui ci si appoggia
...
def test_real_panel():
    print("\n[4] il pannello vero: corr(Q) quasi-diagonale => effetti trascurabili")
    try:
    ...
    _check("il coupling comprerebbe < 2% di calibrazione sulla Q dell'EM",
           oc["overconfidence"] < 0.02, f"{100*oc['overconfidence']:.2f}%")
    _check("l'attenuazione di rho (Branch B) < 1% sulla Q dell'EM",
...
def test_gate1_diagnostics_do_not_move_the_draws():
    print("\n[5] le diagnostiche non consumano RNG (hook di pura strumentazione)")
    from mcmc.gibbs import fit_dfm_mcmc, load_warm_init
    from mcmc.simulate_sv import simulate_dfm_sv
    ...
    # ... e il hook riporta davvero cio' che serve.
    d = on["diagnostics"]
...
def main():
    print("=" * 72)
    print("DIAGNOSTICHE -- metriche su Q + ESS/R-hat + la garanzia no-RNG")
    print("=" * 72)
