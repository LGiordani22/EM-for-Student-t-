# `_archive_mcmc/` — i motori del campionatore (ARCHIVIATO)

> **ARCHIVIATO il 2026-08-06.** Era `src/mcmc/`. Non fa parte della consegna e
> **non è importabile così com'è**: i moduli usano import di primo livello
> (`from mcmc.gibbs import …`, non `from src.mcmc.gibbs import …`) e dipendono
> dal binario legacy `small`/`big`, cancellato il 2026-08-03. Per rimetterlo in
> piedi servirebbe migrarlo a `config/series_final.json`.
>
> Resta versionato perché la teoria corrispondente — Gibbs, stochastic
> volatility, leverage, ASIS — è scritta in `docs/tesi/EM_for_student_t.tex` e i
> risultati di mescolamento sono materiale di tesi.
>
> **Nessuna dipendenza entrante.** L'unica che c'era, `ess` usata da
> `src/bvar/tests/test_mixing.py`, è stata copiata in `src/bvar/diagnostics.py`.
> Il testo qui sotto è quello originale e parla ancora di `src/mcmc/`.
...
Le equazioni citate qui (`eq:...`) stanno in `docs/tesi/EM_for_student_t.tex`.
...
* `docs/tesi/EM_for_student_t.tex` — la teoria: tutte le `eq:...` e le derivazioni dei conditional.
