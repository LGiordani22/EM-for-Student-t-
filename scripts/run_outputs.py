...
def _steps(n_cells: int) -> list[dict]:
    ...
    return [
        ...
        {"nome": "raccolta", "cmds": [
            ["-m", "src.forecast.collect"],
        ]},
        {"nome": "celle", "cmds": [
            ["-m", "src.forecast.test_cells_produced", "--expect", str(n_cells)],
        ]},
        {"nome": "figure", "cmds": figure, "tollera": _VUOTO},
        {"nome": "nyfed", "cmds": [
            ["-m", "src.forecast.nyfed_all", "--spec", s] for s in layout.SPECS
        ], "fatale": False},
        {"nome": "bvar-metrics", "cmds": [["-m", "src.bvar.metrics"]]},
        {"nome": "tabelle", "cmds": [["-m", "src.forecast.metrics_tables"]]},
        {"nome": "allineamento", "cmds": [["-m", "src.forecast.test_windows"]]},
    ]


def _slug(argv: list[str]) -> str:
    """Un nome di file leggibile a partire dagli argomenti del sottoprocesso."""
    pezzi = [p.replace("src.", "").replace(".", "_")
